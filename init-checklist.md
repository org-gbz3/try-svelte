...
- [ ] GitHub トップ → `New repository` をクリックした
- [ ] `Repository template` で `template-default` を選択した
- [ ] リポジトリ名・可視性（Public / Private）を設定した
- [ ] `Create repository` をクリックした
- [ ] Part 1 のデフォルト設定（Dependabot 等）が自動適用されていることを確認した
...
- [ ] Dependabot for Actions を設定した（SHA ピンニングと組み合わせて最新 SHA に自動更新）
